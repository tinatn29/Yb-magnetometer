...
for theta in thetas:
	job_file = os.path.join(os.getcwd(),f"./jobs/{delta_mod:.0f}_fixed1000_{theta:.0f}deg_{num_freqs}bands_bx{bx:.1f}_by{by:.1f}.sh")

	with open(job_file, 'w') as fh:
		fh.writelines("#!/bin/bash\n")
		fh.writelines(f"#SBATCH --job-name={delta_mod:.0f}_fixed1000_{theta:.0f}deg_{num_freqs}bands_bx{bx:.1f}_by{by:.1f}.job\n")
		fh.writelines(f"#SBATCH --output=./out/{delta_mod:.0f}_fixed1000_{theta:.0f}deg_{num_freqs}bands_bx{bx:.1f}_by{by:.1f}.txt\n")
		fh.writelines(f"#SBATCH --error=./err/{delta_mod:.0f}_fixed1000_{theta:.0f}deg_{num_freqs}bands_bx{bx:.1f}_by{by:.1f}.txt\n")
		...
		fh.writelines([f"python3 $SCRATCH/AT_data/RunMC_Josh_bxby_fixed1000.py {delta_mod} {theta} ", '"', light_fields, '" ',\
			f"{bx} {by} {delta_mod:.0f}_fixed1000_{theta:.0f}deg_{num_freqs}bands_bx{bx:.1f}_by{by:.1f}.csv"])
